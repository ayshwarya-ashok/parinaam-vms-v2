...
| `decommission-v1-vm.sh` | Runs **on the shared VM**, not here. Backs up and then removes the legacy `parinaam-vms` (v1) app layer — its containers, `appdb`, and the `Parinaam VMS *` workflows/credentials it left in the shared n8n — so v2 can be deployed onto a clean box. Leaves `/opt/parinam` (shared Postgres/n8n/NocoDB) running; v2 doesn't need it. | Once, before v1 → v2 cutover |
| `vm-setup.sh` | Runs **on the Parinaam VM**: checkout from the streamed git bundle into `/opt/parinaam-vms-v2`, generates a fresh-secrets loopback-bound `.env`, first `docker compose --profile app up`. Idempotent. | Once per VM (and safe to re-run) |
| `vm-go-public.sh` | Runs **on the Parinaam VM** once `vms.parinaam.ai` DNS exists: validates resolution, backs up `/etc/caddy/Caddyfile`, appends the v2 site block, `caddy validate` **before** `systemctl reload`, smoke-tests both hostnames. | Once, when the DNS record lands |
